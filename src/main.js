...
$("#createForm").onsubmit = async (e) => { e.preventDefault(); if (!writeClient) return flash("Connect wallet first.", "error"); const f = new FormData(e.target); const date = new Date(f.get("deadline")).toISOString().replace(".000", ""); try { const txHash = await writeClient.writeContract({ address, functionName: "create_proposal", args: [f.get("title"), f.get("url"), f.get("hash"), BigInt(f.get("quorum")), date] }); flash(`Proposal transaction sent: ${txHash.slice(0, 12)}…`); const receipt = await readClient.waitForTransactionReceipt({ hash: txHash, status: "FINALIZED", fullTransaction: true }); const readable = String(receipt.consensus_data?.leader_receipt?.[0]?.result?.payload?.readable || ""); const match = readable.match(/\d+/); if (!match) throw new Error("Could not read the new proposal ID from the finalized receipt."); const proposalId = BigInt(match[0]); const snapshotHash = await writeClient.writeContract({ address, functionName: "capture_snapshot", args: [proposalId] }); await readClient.waitForTransactionReceipt({ hash: snapshotHash, status: "FINALIZED", fullTransaction: true }); flash(`Proposal ${proposalId} locked and approved snapshot captured.`, "success"); } catch (err) { flash(err.shortMessage || err.message, "error"); } };
...
async function writeAction(method, args) { if (!writeClient) return flash("Connect wallet first.", "error"); try { const txHash = await writeClient.writeContract({ address, functionName: method, args }); flash(`Transaction sent: ${txHash.slice(0, 12)}…`); const receipt = await readClient.waitForTransactionReceipt({ hash: txHash, status: "FINALIZED", fullTransaction: true }); if (receipt.statusName && receipt.statusName !== "FINALIZED") throw new Error(`Transaction ${receipt.statusName}`); flash(`${method} finalized.`, "success"); await loadProposal(); if (method === "verify_context") await loadReceipt(currentId(), txHash); } catch (err) { flash(err.shortMessage || err.message, "error"); } }
async function loadReceipt(id, txHash = "") { if (!readClient || id === "") return; const raw = String(await readClient.readContract({ address, functionName: "proposal_receipt", args: [BigInt(id)], stateStatus: "accepted" })); const [status, verdict, note, execution] = raw.split("|"); const target = $("#verifyDetails"); target.className = "details"; target.innerHTML += `<div class="receipt-readback"><strong>Stored receipt</strong><span>${status} · ${verdict}</span><small>${note || "—"}${execution ? ` · execution: ${execution}` : ""}</small>${txHash ? `<a href="https://explorer-studio.genlayer.com/tx/${txHash}" target="_blank" rel="noreferrer">View finalized transaction ↗</a>` : ""}</div>`; }
